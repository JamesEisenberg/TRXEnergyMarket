...
function joinAction(role)
{
	if (role === 'buyer')
 	{
        	showToast("You have selected to join as a Buyer.", "success");
        } 
	else if (role === 'seller')
	{
                showToast("You have selected to join as a Seller.", "success");
        }
}
...
async function connectWallet() {
    ...
    if (typeof window.tronWeb === 'undefined') {
        tronWebStatusElement.textContent = 'TronLink is not installed.';
    ...
    try {
	 ...
	 // Check if res is undefined or null
        if (!res) {
            showToast("Not Connected: Wallet is locked or unavailable.", "error");
	    ...
	    showToast("Not Connected: " + res.message, "error");
